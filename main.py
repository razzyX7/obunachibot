...
mail_bt = InlineKeyboardButton(text = '✉️ Rassilka bro', callback_data = 'mail')
give_uban_bt = InlineKeyboardButton(text = '🚷 Ban berish/olish', callback_data = 'uban')
change_balance_bt = InlineKeyboardButton(text = '💳 Balansni ozgartirish', callback_data = 'chb')
admin_menu.add(statistics_bt, mail_bt)
admin_menu.add(give_uban_bt, change_balance_bt)

cancel_menu = InlineKeyboardMarkup()
cancel_bt = InlineKeyboardButton(text = '🚫 Bekor qilish', callback_data = 'cancel')
...
@dp.message_handler(lambda m: m.text == '👤 Profil' and user_banned(m.from_user.id) == False)
async def profile_button_handle(m: types.Message):
    await m.reply(PROFILE(m), reply = False, parse_mode = 'Markdown')
    
@dp.message_handler(lambda m: m.text == '➕ Obunachilarni olish' and user_banned(m.from_user.id) == False)
...
async def handle_get_sub_count(m: types.Message):
    if (m.content_type == 'text') and (m.text.isdigit() == True) and (int(m.text) >= LITTLE_SUBCOIN_TO_GET_SUBS) and user_balance(m.from_user.id) >= int(m.text):
        ...
        cancel_wnum_menu.add(InlineKeyboardButton(text = '🚫 Bekor qilish', callback_data = 'cancel_' + str(channel_stat[-1])))
        await m.reply(LITTLE_SUBCOIN_2, reply = False, reply_markup = cancel_wnum_menu)
        
@dp.message_handler(lambda m: m.text == '✔️ Kanallarga obuna bolish' and user_banned(m.from_user.id) == False)
            ...
            if channel_to_subscribe not in black_list:
                ...
                if bot_status == "administrator":
                    ...
                    if status_of_user.status == 'left':
                        ...
                        subscribe_menu.add(InlineKeyboardButton(text = 'Kanalga otish', url = 'tg://resolve?domain=' + username.username))
                        subscribe_menu.add(InlineKeyboardButton(text = 'Obunani tekshirish', callback_data = 'sub_' + str(channels_list[channel_to_subscribe])))
...
@dp.message_handler(lambda m: m.text == '👣 Hamkorlik dasturi' and user_banned(m.from_user.id) == False)
...
if __name__ == '__main__':
    executor.start_polling(dp, skip_updates=True, host="0.0.0.0", port=int(os.environ.get("PORT", 10000)))
